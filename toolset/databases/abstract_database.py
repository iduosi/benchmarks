...
class AbstractDatabase:
    ...
    def verify_queries(cls, config, table_name, url, concurrency=512, count=2, check_updates=False):
        ...
        Retrieve from the database statistics of the number of queries made,
          the number of rows read, eventually the number of updated rows.
        ...
        path = config.db_root
        process = PopenTimeout(shlex.split("siege -c %s -r %s %s -R %s/.siegerc" % (concurrency, count, url, path)),
