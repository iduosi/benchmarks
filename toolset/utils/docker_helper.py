...
class DockerHelper:
            ...
            try:
                ...
                for token in output:
                    ...
                    while "\n" in buffer:
                        ...
                        log(line, prefix=log_prefix, file=build_log,
                            color=Fore.WHITE + Style.BRIGHT if re.match(r'^Step \d+\/\d+', line) else '')
                    ...
                    if self.benchmarker.time_logger.time_since_start() > 3600:
                        log("Build time exceeded 60 minutes", prefix=log_prefix, file=build_log, color=Fore.RED)
                        raise Exception

                if buffer:
                    log(buffer, prefix=log_prefix, file=build_log,
                        color=Fore.WHITE + Style.BRIGHT if re.match(r'^Step \d+\/\d+', buffer) else '')
            except Exception:
                tb = traceback.format_exc()
                log("Docker build failed; terminating", prefix=log_prefix, file=build_log, color=Fore.RED)
                log(tb, prefix=log_prefix, file=build_log)
                self.benchmarker.time_logger.log_build_end(log_prefix=log_prefix, file=build_log)
                raise
    ...
    def clean(self):
        ...
        for image in self.server.images.list():
            if len(image.tags) > 0:
                ...
                if image_tag != 'iduosi/benchmark' and 'iduosi' in image_tag:
    ...
    def build(self, test, build_log_dir=os.devnull):
        ...
        try:
            self.__build(
                ...
                tag="iduosi/benchmark.test.%s" % test.name)
        except Exception:
            return 1
        ...
        try:
            ...
            def watch_container(docker_container, docker_file):
                with open(os.path.join(
                        run_log_dir, "%s.log" % docker_file.replace(".dockerfile", "").lower()), 'w') as run_log:
            ...
            container = self.server.containers.run("iduosi/benchmark.test.%s" % test.name,
                                                   name=name,
                                                   command=docker_cmd,
            ...
            watch_thread = Thread(target=watch_container, args=(container, "%s.dockerfile" % test.name, ))
            ...
            with open(os.path.join(run_log_dir, "%s.log" % test.name.lower()), 'w') as run_log:
                tb = traceback.format_exc()
                log("Running docker container: %s.dockerfile failed" % test.name, prefix=log_prefix, file=run_log)
    ...
    def __stop_all(docker_client):
        for container in docker_client.containers.list():
            if len(container.image.tags) > 0 \
                    and 'iduosi' in container.image.tags[0] \
                    and 'benchmark:latest' not in container.image.tags[0]:
    ...
    def build_databases(self):
        ...
        for test in self.benchmarker.tests:
            ...
            if db not in built and db != "none":
                ...
                self.__build(base_url=self.benchmarker.config.database_docker_host,
                             path=database_dir,
    ...
    def start_database(self, database):
        ...
        container.
        """
        image_name = "iduosi/%s:latest" % database
        log_prefix = image_name + ": "
    ...
    def build_wrk(self):
        """Builds the iduosi/benchmark.wrk container"""
        self.__build(base_url=self.benchmarker.config.client_docker_host,
                     path=self.benchmarker.config.wrk_root,
                     dockerfile="Dockerfile",
                     log_prefix="wrk: ",
                     build_log_file=os.devnull,
                     tag="iduosi/benchmark.wrk")
