...
class Metadata:
    ...
    def gather_language_tests(self, language):
        ...
        try:
            ...
            return filter(lambda x: os.path.isdir(os.path.join(self.benchmarker.config.lang_root, x)), tests)
        except Exception:
            raise Exception("Unable to locate language directory: {!s}".format(language))
    ...
    def get_framework_config(self, test_dir):
        ...
        test directory
        """
        dir_config_files = glob.glob("{!s}/{!s}/benchmark_config.json".format(self.benchmarker.config.lang_root, test_dir))
    ...
    def gather_tests(self, include=None, exclude=None):
        ...
        if self.benchmarker.config.test_dir:
            ...
            config_files.extend(
                glob.glob("{!s}/*/*/benchmark_config.json".format(self.benchmarker.config.lang_root)))
        ...
        for config_file_name in config_files:
            ...
            with open(config_file_name, 'r') as config_file:
                try:
                    ...
                    log("Error loading config: {!s}".format(config_file_name), color=Fore.RED)
    ...
    def tests_to_run(self):
        ...
        """
        return self.gather_tests(self.benchmarker.config.test, self.benchmarker.config.exclude)
    ...
    def parse_config(self, config, directory):
        ...
        for test in config['tests']:
            ...
            for test_name, test_keys in test.iteritems():
                ...
                tests.append(
                    FrameworkTest(test_name, directory, self.benchmarker, sortedRunTests, test_keys))
